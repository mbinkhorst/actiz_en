...
(function() {
	...
	Chick.registerController('Actiz.Mobile.BlogController', BlogController);

	BlogController.prototype.index = function() {

		var view = Chick.Gui.View.make('mobile/blog/index')
	...
	BlogController.prototype.show = function(slug) {
		...
		var view = Chick.Gui.View.make('mobile/blog/show')
			...
			.on('ready' ,function() {
				...
				window.disqus_title = 'HBO-V in de ouderenzorg - Daar zit meer achter';
		 		...
		 		// Set url to be the same as desktop
	...
	BlogController.prototype.showAuthor = function(slug) {

		var view = Chick.Gui.View.make('mobile/blog/show-author')

			.withCollection('posts', Chick.api('/posts'), function(result) {

				// No author by this slug?
				var post = result.findBy('author.slug', slug);
				if (post === undefined) {
					return false;
				}
				this.data.author = post.get('author');


				// Find author's posts
				var self = this;
				this.data.author.posts = result.filter(function(post) {
					return post.get('author.id') === self.data.author.get('id');
				});
			...
			.on('ready' ,function() {
				...
				// Toggle
				var $btnToggle = $details.siblings('.toggle');
